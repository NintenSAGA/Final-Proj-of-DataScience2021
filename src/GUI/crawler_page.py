...
class Panel:
    ...
    def create_src_option(self):
        Label(self.canvas, text='文书源', font=get_ft(20), bg='white').place(x=50, y=25)
        ...
        for i, site_pack in enumerate(src_sites):
            var, site = site_pack
            bt = Radiobutton(self.canvas, text=site, variable=self.src_site, value=var, font=get_ft(14),
    ...
    def create_entry(self):
        ...
        for i, pack in enumerate(entries):
            text, var, entry = pack
            Label(self.canvas, text=text, font=get_ft(15), bg='white').place(x=lst_x + 150 * i, y=lst_y)
            entry = Entry(self.canvas, width=5, borderwidth=4, textvariable=var)
            entry.place(x=est_x + i * 170, y=est_y)

    def create_launch_bt(self):
        self.launch_bt = Button(self.canvas, text='开始爬取', font=get_ft(14), bg='grey', command=self.begin, pady=0)
    ...
    def create_crawler_option(self):
        Label(self.canvas, text='爬取选项', font=get_ft(20), bg='white').place(x=50, y=200)
        ...
        for i, pack in enumerate(choices):
            text, var, cmd = pack
            bt = Checkbutton(self.canvas, text='爬取' + text, variable=var, font=get_ft(14))
            bt.place(x=st_x, y=st_y + 40 * i)
            var.trace('w', lambda *args: self.url_list_action())
            label = Label(self.canvas, text='', font=get_ft(10), bg='white', fg='blue')
